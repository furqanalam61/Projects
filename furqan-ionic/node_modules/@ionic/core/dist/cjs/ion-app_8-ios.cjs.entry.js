...
const index = require('./index-a179445b.js');
    ...
    componentDidLoad() {
        {
            helpers.rIC(() => {
                ...
                if (!config.config.getBoolean('_testing')) {
                    new Promise(function (resolve) { resolve(require('./tap-click-402a66cb.js')); }).then(module => module.startTapClick(config.config));
                }
                if (config.config.getBoolean('statusTap', isHybrid)) {
                    new Promise(function (resolve) { resolve(require('./status-tap-6652c394.js')); }).then(module => module.startStatusTap());
                }
                if (config.config.getBoolean('inputShims', needInputShims())) {
                    new Promise(function (resolve) { resolve(require('./input-shims-876a8164.js')); }).then(module => module.startInputShims(config.config));
                }
...
const Content = class {
    ...
    shouldForceOverscroll() {
        ...
        return forceOverscroll === undefined
            ? mode === 'ios' && config.isPlatform(window, 'mobile')
            : forceOverscroll;
    }
    resize() {
        if (this.fullscreen) {
